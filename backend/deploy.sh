...
if [[ "$SKIP_MIGRATE" == "false" ]]; then
  ...
  source "$ENV_FILE" 2>/dev/null || true   # load DB vars
  ...
  for migration in \
    ...
    if [[ -f "$REPO_ROOT/$migration" ]]; then
      ...
      $DC exec -T postgres psql \
        ...
        -v ON_ERROR_STOP=0 \
        -f "/dev/stdin" \
        < "$REPO_ROOT/$migration" 2>&1 | grep -v "already exists" || true
    else
...
for i in {1..15}; do
  ...
  [[ $i -eq 15 ]] && die "Backend not healthy after 30s — check: $DC logs backend"
  sleep 2
done
