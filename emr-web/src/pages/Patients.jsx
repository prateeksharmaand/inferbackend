...
function AddPatientAbhaModal({ onClose, onSuccess }) {
  const [searchType, setSearchType] = useState('abha_address'); // abha_address | abha_number | mobile | aadhaar
  const [step, setStep]             = useState('search');     // search | otp | done
  ...
  const requestOtp = async () => {
    if (!searchValue.trim()) return toast.error(`Enter ${searchType.replace(/_/g, ' ')}`);
    setLoading(true);
    try {
      const payload = {};
      if (searchType === 'abha_address') payload.abhaId = searchValue.trim();
      else if (searchType === 'abha_number') payload.abhaId = searchValue.trim();
      else if (searchType === 'mobile') payload.mobile = searchValue.trim();
      else if (searchType === 'aadhaar') payload.aadhaar = searchValue.trim();
  ...
  const inp = { width: '100%', padding: '10px 14px', borderRadius: 6, border: '1.5px solid #e2e8f0', fontSize: 13, outline: 'none', boxSizing: 'border-box' };
  const btn = (color = '#7c3aed') => ({ width: '100%', padding: '10px 16px', borderRadius: 6, border: 'none', background: color, color: '#fff', fontWeight: 600, fontSize: 13, cursor: 'pointer', opacity: loading ? 0.6 : 1 });
  ...
  return (
    <div onClick={e => e.target === e.currentTarget && onClose()} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 1000 }}>
      <div style={{ background: '#fff', borderRadius: 12, padding: '28px', width: 420, maxHeight: '90vh', overflow: 'auto', boxShadow: '0 20px 60px rgba(0,0,0,0.3)', position: 'relative' }}>
        <button onClick={onClose} style={{ position: 'absolute', top: 14, right: 14, background: 'none', border: 'none', cursor: 'pointer', color: '#94a3b8', fontSize: 18 }}>×</button>

        <h2 style={{ margin: '0 0 14px', fontSize: 18, fontWeight: 700, color: '#1e293b' }}>New Patient Registration Using ABHA</h2>
        ...
        <div style={{ display: 'flex', gap: 8, marginBottom: 20, flexWrap: 'wrap' }}>
          {[
            ...
            <button
              key={key}
              onClick={() => { setSearchType(key); setSearchValue(''); setOtp(''); }}
              style={{
                padding: '6px 14px', borderRadius: 20, border: `1.5px solid ${searchType === key ? '#7c3aed' : '#e2e8f0'}`,
                background: searchType === key ? '#f3e8ff' : '#fff', color: searchType === key ? '#7c3aed' : '#64748b',
                fontWeight: searchType === key ? 600 : 500, fontSize: 12, cursor: 'pointer',
              }}
            >
              {label}
            </button>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <div>
              ...
              <input
                style={inp}
                placeholder={
                  searchType === 'abha_address' ? 'e.g. username@abdm'
                  : searchType === 'abha_number' ? 'e.g. 91-2345-6789-0123'
                  : searchType === 'mobile' ? 'e.g. 9876543210'
                  : 'e.g. 1234-5678-9012'
                }
                value={searchValue}
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button style={btn('#f97316')} onClick={requestOtp} disabled={loading}>{loading ? 'Sending OTP…' : 'Verify ABHA'}</button>
              <button style={btn('#dc2626')} onClick={onClose}>CANCEL</button>
            </div>
          </div>
        )}
        ...
        {step === 'otp' && (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <div style={{ background: '#eff6ff', borderRadius: 8, padding: '12px', fontSize: 12, color: '#1d4ed8', display: 'flex', alignItems: 'center', gap: 8 }}>
              <AlertCircle size={14} />
              OTP sent to registered mobile number
            </div>
            <div>
              <label style={{ display: 'block', fontSize: 12, fontWeight: 600, color: '#475569', marginBottom: 6 }}>Enter OTP</label>
              <input style={inp} placeholder="6-digit OTP" value={otp} onChange={e => setOtp(e.target.value.slice(0, 6))} maxLength={6} />
            </div>
            <div style={{ display: 'flex', gap: 10 }}>
              <button style={btn('#f97316')} onClick={verifyOtp} disabled={loading}>{loading ? 'Verifying…' : 'Verify & Create'}</button>
              <button style={btn('#dc2626')} onClick={() => setStep('search')}>BACK</button>
            </div>
          </div>
        )}
        ...
        {step === 'done' && newPatient && (
          <div style={{ textAlign: 'center', display: 'flex', flexDirection: 'column', gap: 14 }}>
            <div style={{ width: 56, height: 56, background: '#f0fdf4', borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto' }}>
              <Check size={28} color="#16a34a" strokeWidth={3} />
            </div>
            <div>
              <p style={{ margin: '0 0 6px', fontWeight: 700, fontSize: 15, color: '#16a34a' }}>Patient Created Successfully!</p>
